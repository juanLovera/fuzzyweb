...
function cambiarPagina(l, o, p)
{
    ...
    document.getElementById('selected').onmouseout = function () {
...
{
    $("#modal-html").slideUp("fast", function () {
        $("#modal-loading").show();
        rq = "perfil_u.php?idUsuario=" + id_usuario;
        jqaj = $.ajax(rq)
                .done(function (res) {
                    $("#modal-html").slideDown("fast");
                    $("#modal-loading").hide();
                    document.getElementById("modal-html").innerHTML = res;
                })
                .fail(function () {
                    alert("Ha ocurrido un error al enviar el formulario.");
                    $("#modal-html").slideDown("fast");
